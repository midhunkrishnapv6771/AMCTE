...
try:
    from salesman_state import get_apify_quota as _get_apify_quota
...
def _get_instagram_cookies(user_id: Optional[str] = None) -> Optional[str]:
    """
    Retrieves Instagram cookies to authenticate Apify scraper.
    Checks:
      1. user_id specific cookie from Credentials pool
      2. APIFY_INSTAGRAM_COOKIES or INSTAGRAM_COOKIES env variable
      3. Mapped COOKIES_FILE (by parsing its Netscape or JSON format)
      4. Rotated cookie from Credentials pool
    """
    # 1. Check user-specific credentials
    if user_id:
        try:
            ...
            cookie = creds.get("instagram_cookie", "")
            if cookie:
                logger.info("🍪 Found user-specific Instagram cookies in pool for user: %s", user_id)
                return cookie
        except Exception as e:
            logger.error("💥 Error fetching user-specific cookies: %s", e)
    ...
    # 4. Check rotated credentials from pool
    try:
        from Telegram_credentials_handler_modules.credential_pool_manager import pool_manager
        creds = pool_manager.get_next_rotated_credentials()
        cookie = creds.get("instagram_cookie", "")
        if cookie:
            logger.info("🍪 Found rotated Instagram cookie in pool")
            return cookie
    except Exception as e:
        logger.error("💥 Error fetching rotated cookies: %s", e)
...
def _get_client(user_id: Optional[str] = None):
    ...
    # 1. Try user-specific token from pool
    if user_id:
        try:
            from Telegram_credentials_handler_modules.credential_pool_manager import pool_manager
            creds = pool_manager.get_user_credentials(user_id)
            ...
            if token:
                logger.info("🔑 Using user-specific Apify API Token for user: %s", user_id)
        except Exception as e:
            logger.error("💥 Error fetching user-specific token: %s", e)
            
    # 2. Try default environment token
    if not token:
        token = os.getenv("APIFY_API_TOKEN", "").strip()
        if token:
            logger.debug("🔑 Using environment default Apify API Token")
            
    # 3. Try rotated token from pool
    if not token:
        try:
            from Telegram_credentials_handler_modules.credential_pool_manager import pool_manager
            creds = pool_manager.get_next_rotated_credentials()
            token = creds.get("apify_token", "")
            if token:
                logger.info("🔑 Using rotated Apify API Token from pool")
        except Exception as e:
            logger.error("💥 Error fetching rotated token: %s", e)
            
    if not token:
        raise RuntimeError(
            "APIFY_API_TOKEN is not set. Please provide it in environment or user credentials pool."
        )
...
def apify_get_video_url(instagram_url: str, user_id: Optional[str] = None) -> Optional[str]:
    """
    ...
    try:
        ...
        cookies = _get_instagram_cookies(user_id=user_id)
        if cookies:
...
def apify_scrape_actress_accounts(
    ...
    try:
        ...
        try:
            ...
            try:
                from actress_ledger import get_ledger
                _ledger = get_ledger()
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="🎬  AMTCE Apify Integration Module — run Apify scrapes and lookups"
    )
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--url", "-u", help="Get direct CDN URL for a single Instagram post/reel")
    group.add_argument("--account", "-a", help="Scrape a specific Instagram account (comma-separated list)")
    
    parser.add_argument("--limit", "-l", type=int, default=3, help="Limit per account (default: 3)")
    parser.add_argument("--actress", type=str, default="Unknown", help="Target actress/subject name (default: Unknown)")
    ...
    if args.url:
        print(f"\n🔍 Querying Apify for single URL: {args.url}\n")
        cdn_url = apify_get_video_url(args.url)
        if cdn_url:
            print(f"✅ Success! Direct CDN URL:\n{cdn_url}\n")
        else:
            print("❌ Failed to retrieve CDN URL or quota exceeded.")
            sys.exit(1)
            
    elif args.account:
        accounts = [acc.strip() for acc in args.account.split(",") if acc.strip()]
        print(f"\n🔍 Scraping accounts: {accounts} for actress: {args.actress} (limit: {args.limit})\n")
        results = apify_scrape_actress_accounts(args.actress, accounts, limit_per_account=args.limit)
        print(f"✅ Completed! Found {len(results)} new reels:\n")
        for item in results:
            print(f"- Shortcode: {item.get('shortcode')}")
            print(f"  Owner: {item.get('ownerUsername')}")
            print(f"  URL: {item.get('url')}")
            print(f"  Video CDN: {item.get('videoUrl')}")
