...
# ---------------------------------------------------------------------------
try:
    from Media_guards.smart_reuse_engine import check_smart_reuse, register_video
...
_COOKIES_FILE = os.getenv("COOKIES_FILE", os.path.join(PROJECT_ROOT, "cookies.txt"))
...
def _update_yt_dlp() -> bool:
    ...
    with _UPDATE_LOCK:
        ...
        # Set cooldown time immediately to prevent double-triggering on errors
        ...
        try:
            ...
            logger.info("✅ yt-dlp package updated on disk. Please restart the app/server to load the new version.")
            return True
...
_DB_DIR = os.path.join(_MODULE_DIR, "Downloader_db")
os.makedirs(_DB_DIR, exist_ok=True)
_DB_PATH = os.path.join(_DB_DIR, "index.db")
...
class DownloadIndex:
    ...
    _backup_cooldown = 300.0  # 5-minute cooldown to prevent spamming Telegram backups

    @classmethod
    def _ensure_schema(cls):
        # Startup recovery: attempt to download the index.db backup from Telegram
        if not cls._RESTORED:
            cls._RESTORED = True
            try:
                from Downloader_db.telegram_backup_for_downloads import TelegramBackupManager
                tg = TelegramBackupManager()
                if tg.enabled:
                    if not os.path.exists(_DB_PATH) or os.getenv("GITHUB_ACTIONS") == "true":
                        tg.restore_database(_DB_PATH)
            except Exception as e:
                logger.error("[TELEGRAM] Startup database restore failed: %s", e)
        ...
        with _get_db() as db:
            ...
            for col_name, col_type in [("telegram_video_file_id", "TEXT"), ("telegram_audio_file_id", "TEXT")]:
                try:
                    db.execute(f"ALTER TABLE downloads ADD COLUMN {col_name} {col_type}")
    ...
    @classmethod
    def trigger_backup(cls, force=False):
        try:
            from Downloader_db.telegram_backup_for_downloads import TelegramBackupManager
            tg = TelegramBackupManager()
            if not tg.enabled:
            ...
            import time
            now = time.time()
            if not force and (now - cls._last_backup_time < cls._backup_cooldown):
                logger.info("[TELEGRAM] Database backup skipped (throttling active, within 5-minute cooldown).")
                return
                
            tg.backup_database(_DB_PATH)
            cls._last_backup_time = now
        except Exception as e:
            logger.error("[TELEGRAM] Database backup failed: %s", e)
    ...
    @classmethod
    def register_telegram_ids(cls, url_id: str, video_file_id: Optional[str] = None, audio_file_id: Optional[str] = None) -> None:
        cls._ensure_schema()
        if not url_id: return
        with _get_db() as db:
            if video_file_id:
                db.execute("UPDATE downloads SET telegram_video_file_id=? WHERE url_id=?", (video_file_id, str(url_id)))
            if audio_file_id:
                db.execute("UPDATE downloads SET telegram_audio_file_id=? WHERE url_id=?", (audio_file_id, str(url_id)))
        cls.trigger_backup()

    @classmethod
    def _ensure_telegram_audio(cls, url_id: str):
        try:
            with _get_db() as db:
                row = db.execute(
                    "SELECT telegram_audio_file_id FROM downloads WHERE url_id=? LIMIT 1", (str(url_id),)
                ).fetchone()
            if row and row[0]:
                telegram_audio_file_id = row[0]
                with _get_db() as db:
                    v_row = db.execute("SELECT file_path FROM downloads WHERE url_id=?", (str(url_id),)).fetchone()
                if v_row:
                    v_path = v_row[0]
                    v_base = os.path.splitext(os.path.basename(v_path))[0]
                    a_path = os.path.abspath(os.path.join(PROJECT_ROOT, "Original_audio", f"{v_base}.mp3"))
                    if not os.path.exists(a_path):
                        from Downloader_db.telegram_backup_for_downloads import TelegramBackupManager
                        tg = TelegramBackupManager()
                        if tg.enabled:
                            logger.info("☁️  Restoring original audio from Telegram: %s", os.path.basename(a_path))
                            tg.download_file(telegram_audio_file_id, a_path)
        except Exception as e:
            logger.error("[TELEGRAM] Failed to restore audio from cloud: %s", e)
    ...
    def find_by_id(cls, url_id: str) -> Optional[str]:
        ...
        with _get_db() as db:
            row = db.execute(
                "SELECT file_path, telegram_video_file_id FROM downloads WHERE url_id=? LIMIT 1", (str(url_id),)
            ).fetchone()
        if not row:
            return None
        
        file_path, telegram_video_file_id = row
        if os.path.exists(file_path):
            return file_path
            
        if telegram_video_file_id:
            try:
                from Downloader_db.telegram_backup_for_downloads import TelegramBackupManager
                tg = TelegramBackupManager()
                if tg.enabled:
                    logger.info("☁️  Restoring clip from Telegram cloud storage: %s", os.path.basename(file_path))
                    if tg.download_file(telegram_video_file_id, file_path):
                        cls._ensure_telegram_audio(url_id)
                        return file_path
            except Exception as e:
                logger.error("[TELEGRAM] Failed to restore clip from cloud: %s", e)
        return None
    ...
    def find_by_hash(cls, target_hash: str) -> Optional[str]:
        ...
        with _get_db() as db:
            row = db.execute(
                "SELECT file_path, telegram_video_file_id, url_id FROM downloads WHERE content_hash=? LIMIT 1", (target_hash,)
            ).fetchone()
        if not row:
            return None
            
        file_path, telegram_video_file_id, url_id = row
        if os.path.exists(file_path):
            return file_path
        ...
        if telegram_video_file_id:
            try:
                ...
                tg = TelegramBackupManager()
                if tg.enabled:
                    logger.info("☁️  Restoring clip from Telegram cloud storage (by hash): %s", os.path.basename(file_path))
                    if tg.download_file(telegram_video_file_id, file_path):
                        cls._ensure_telegram_audio(url_id)
                        return file_path
            except Exception as e:
...
def _get_active_cookie_file(user_id: Optional[str] = None) -> Optional[str]:
    """
    Returns the path to a cookies file to use.
    If user_id is provided, tries to use the user's specific cookie from the pool.
    If no user_id is provided, tries to use the environment's COOKIES_FILE,
    and if that doesn't exist, falls back to a rotated cookie from the pool.
    """
    try:
        ...
        if user_id:
            ...
            cookie_str = creds.get("instagram_cookie", "")
            if cookie_str:
                return pool_manager.get_cookie_file_path(user_id, cookie_str)
            logger.info("ℹ️  No custom cookie found in pool for user: %s", user_id)
            
        # Try default environment file first
        if os.path.exists(_COOKIES_FILE) and os.path.getsize(_COOKIES_FILE) > 100:
            logger.debug("🍪 Using environment default cookies file: %s", _COOKIES_FILE)
            return _COOKIES_FILE
            
        # Try rotated credentials from pool
        creds = pool_manager.get_next_rotated_credentials()
        cookie_str = creds.get("instagram_cookie", "")
        if cookie_str:
            return pool_manager.get_cookie_file_path("rotated", cookie_str)
            
    except Exception as e:
        logger.error("💥 Error resolving active cookie file: %s", e)
        
    return None

def _build_strategy_opts(base_opts: Dict, strategy: str, user_id: Optional[str] = None) -> Optional[Dict]:
    ...
    if strategy == "cookies_file":
        cookie_file = _get_active_cookie_file(user_id)
...
def download_video(
    ...
    for strategy in _AUTH_STRATEGIES:
        ...
        opts = _build_strategy_opts(base_ydl_opts, strategy, user_id=user_id)
        if opts is None:
    ...
    if not success and platform == "instagram" and _is_real_ig_url:
        logger.warning("💰 [TIER 9] All yt-dlp strategies exhausted → Apify fallback")
        try:
            from apify_downloader import apify_get_video_url
            cdn_url = apify_get_video_url(url, user_id=user_id)
            if cdn_url:
    ...
    try:
        ...
        if os.environ.get("SKIP_AUDIO_EXTRACT", "").lower() == "true":
            ...
            try:
                from video_rendering_modules.video_pipeline import extract_audio_from_video
        ...
        # ── Telegram Media Archiving ──
        try:
            from Downloader_db.telegram_backup_for_downloads import TelegramBackupManager
            tg = TelegramBackupManager()
            if tg.enabled:
                logger.info("[TELEGRAM] Archiving downloaded clip to Telegram...")
                v_file_id = tg.upload_media(final_path)
                a_file_id = None
                if not os.environ.get("SKIP_AUDIO_EXTRACT", "").lower() == "true":
                    local_a = locals().get("_audio_out")
                    if local_a and os.path.exists(local_a):
                        a_file_id = tg.upload_media(local_a)
                
                if url_id:
                    DownloadIndex.register_telegram_ids(url_id, video_file_id=v_file_id, audio_file_id=a_file_id)
        except Exception as tg_err:
            logger.warning("⚠️  Telegram media archiving failed (non-fatal): %s", tg_err)
