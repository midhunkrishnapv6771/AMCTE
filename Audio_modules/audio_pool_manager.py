        ...
        try:
            for filename in os.listdir(self.base_dir):
                ...
                if not os.path.isfile(src):
                    continue
                # ── FRESHNESS GUARD ──
                # Skip files modified in the last 10 seconds to avoid stealing audio currently being written/analyzed.
                try:
                    if time.time() - os.path.getmtime(src) < 10:
                        logger.debug(f"[POOL_SYNC] Skipping fresh file: {filename}")
                        continue
                except Exception:
                    pass
